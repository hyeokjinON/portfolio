	...
	function onAjaxError (e){
		alert("[가이드]사진 업로더할 서버URL셋팅이 필요합니다.-onAjaxError "+ e.getMessage());
	}
 	...
 	function callFileUploader (){
 		oFileUploader = new jindo.FileUploader(jindo.$("uploadInputBox"),{
 	        ...
 	        sCallback : '/resources/smartEditor/static/sample/photo_uploader/callback.html',	//업로드 이후에 iframe이 redirect될 콜백페이지의 주소
	 ...
	 */
 	function setPhotoToEditor(oFileInfo){
		if (!!opener && !!opener.nhn && !!opener.nhn.husky && !!opener.nhn.husky.PopUpManager) {
			...
			opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', ["<img style=\"max-width:640px; \" src='../"+oFileInfo[0].sFileURL+"' />"]);
			//본문에 바로 tag를 넣는 방법 (oFileInfo는 String으로 <img src=....> )
			//opener.nhn.husky.PopUpManager.setCallback(window, 'PASTE_HTML', [oFileInfo]);
			
			/*다중이미지 업로드 기능도 원하시면
			for (var i=0; i<oFileInfo.length; i++){} 으로 감싸고 oFileInfo[i] 로 바꿔주세요.
			주의 : 이미지가 엑박으로 나온다면 src 부분을 바꿔주세요*/ 
		}
	}
